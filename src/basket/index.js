...
const { ddbClient } = require("../product/ddbClient");
...
const deleteBasket = async (userName) => {
    console.log(`deleteProduct function. userName : "${userName}"`);
...
const checkoutBasket = async (event) => {
    ...
    // 2- Create an event json object with basket items, calculate total price, prepare order create json data to send order microservice
    var checkoutPayload = prepareOrderPayload(checkoutRequest, basket);

    // 3- Publish event to eventbridge - this will subscribe by order microservice
